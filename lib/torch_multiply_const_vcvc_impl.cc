...
#include "torch_multiply_const_vcvc_impl.h"
...
torch_multiply_const_vcvc::sptr torch_multiply_const_vcvc::make(
    const std::vector<gr_complex>& constant,
    unsigned int device_num) {
    return gnuradio::make_block_sptr<torch_multiply_const_vcvc_impl>(
        constant, device_num);
}
 ...
 */
torch_multiply_const_vcvc_impl::torch_multiply_const_vcvc_impl(
    const std::vector<gr_complex>& constant,
    unsigned int device_num)
    : gr::sync_block(
          "torch_multiply_const_vcvc",
...
int torch_multiply_const_vcvc_impl::work(
    int noutput_items,
    ...
    for (unsigned int idx = 0; idx < noutput_items; idx++) {
        ...
        auto input = torch::from_blob(
            reinterpret_cast<void*>(in), { d_constant_imag.size(0) * 2 }, options);
        input.to(d_device);
