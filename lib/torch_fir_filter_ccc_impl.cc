...
#include "torch_fir_filter_ccc_impl.h"
...
torch_fir_filter_ccc::sptr torch_fir_filter_ccc::make(
    const std::vector<gr_complex>& taps,
    unsigned int downsample_rate,
    unsigned int device_num) {
    return gnuradio::make_block_sptr<torch_fir_filter_ccc_impl>(
        taps, downsample_rate, device_num);
}

/*
 * The private constructor
 */
torch_fir_filter_ccc_impl::torch_fir_filter_ccc_impl(
    const std::vector<gr_complex>& taps,
    unsigned int downsample_rate,
    unsigned int device_num)
    : gr::sync_decimator(
          "torch_fir_filter_ccc",
    ...
    d_conv_options = torch::nn::functional::Conv1dFuncOptions().stride(d_downsample_rate);
}
...
int torch_fir_filter_ccc_impl::work(
    int noutput_items,
    ...
    // We can probably reduce this to two convolutions by putting real/imag in
    // channels instead of entirely different tensors.
    auto conv_ac = torch::nn::functional::conv1d(real_input, d_real_taps, d_conv_options);
    auto conv_ad = torch::nn::functional::conv1d(real_input, d_imag_taps, d_conv_options);
    auto conv_bc = torch::nn::functional::conv1d(imag_input, d_real_taps, d_conv_options);
    auto conv_bd = torch::nn::functional::conv1d(imag_input, d_imag_taps, d_conv_options);
    ...
    for (int i = 0; i < real_output.size(0); i++) {
