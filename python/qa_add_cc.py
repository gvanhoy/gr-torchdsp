...
class qa_add_cc(gr_unittest.TestCase):
    ...
    def test_001_descriptive_test_name(self):
        taps = np.ones((12,), dtype=np.complex64) + 1.0j * np.ones((12,), dtype=np.complex64)
        data = np.exp(1j*np.pi*np.linspace(0, 10000, 10001, endpoint=True)/2000)

        # Torch DSP
        source = blocks.vector_source_c(data, False, 1)
        source2 = blocks.vector_source_c(data, False, 1)
        add_block = add_cc(2, 0)
