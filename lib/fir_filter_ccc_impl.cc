...
fir_filter_ccc::sptr fir_filter_ccc::make(const std::vector<gr_complex> &taps,
                                          unsigned int downsample_rate,
                                          unsigned int device_num) {
  return gnuradio::make_block_sptr<fir_filter_ccc_impl>(taps, downsample_rate,
                                                        device_num);
}
...
int fir_filter_ccc_impl::work(int noutput_items,
  ...
  auto real_input =
      input.index({torch::indexing::Slice(0, torch::indexing::None, 2)})
          .reshape({1, 1, -1});
  auto imag_input =
      input.index({torch::indexing::Slice(1, torch::indexing::None, 2)})
          .reshape({1, 1, -1});

  // We can probably reduce this to two convolutions by putting real/imag in
  // channels instead of entirely different tensors.
  auto conv_ac =
      torch::nn::functional::conv1d(real_input, d_real_taps, d_conv_options);
  auto conv_ad =
      torch::nn::functional::conv1d(real_input, d_imag_taps, d_conv_options);
  auto conv_bc =
      torch::nn::functional::conv1d(imag_input, d_real_taps, d_conv_options);
  auto conv_bd =
      torch::nn::functional::conv1d(imag_input, d_imag_taps, d_conv_options);
