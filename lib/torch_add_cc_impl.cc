...
torch_add_cc::sptr torch_add_cc::make(unsigned int num_inputs, unsigned int device_num) {
    return gnuradio::make_block_sptr<torch_add_cc_impl>(num_inputs, device_num);
}

/*
 * The private constructor
 */
torch_add_cc_impl::torch_add_cc_impl(unsigned int num_inputs, unsigned int device_num)
    : gr::sync_block(
          "torch_add_cc",
 ...
 */
torch_add_cc_impl::~torch_add_cc_impl() {}

int torch_add_cc_impl::work(
    int noutput_items,
    ...
    for (unsigned int input_idx = 0; input_idx < d_num_inputs; input_idx++) {
        input_type* in =
            reinterpret_cast<input_type*>(const_cast<void*>(input_items[input_idx]));
        auto input =
            torch::from_blob(reinterpret_cast<void*>(in), { noutput_items * 2 }, options);
        input.to(d_device);
