...
try:
    from torchdsp import torch_multiply_const_vcvc
except ImportError:
    import os
    import sys
...
class qa_torch_multiply_const_vcvc(gr_unittest.TestCase):
    ...
    def test_instance(self):
        const = np.ones((12,), dtype=np.complex64) + 1.0j * \
            np.ones((12,), dtype=np.complex64)
        instance = torch_multiply_const_vcvc(const, 0)

    def test_001_multiply_const_sinusoid(self):
        const = np.ones((12,), dtype=np.complex64) + 1.0j * \
            np.ones((12,), dtype=np.complex64)
        data = np.exp(1j*np.pi*np.linspace(0, 12000 -
                      1, 12000, endpoint=True)/2000)

        # Torch DSP
        source = blocks.vector_source_c(data, False, 12)
        multiply_const = torch_multiply_const_vcvc(const, 0)
...
if __name__ == '__main__':
    gr_unittest.run(qa_torch_multiply_const_vcvc)
